...
const scrollUp = () => {
  ...
  if (this.scrollY >= 350) {
    ...
    stickyCounter.classList.add("show-sticky-counter");
  } else {
    scrollUpBtn.classList.remove("show-scroll");
    stickyCounter.classList.remove("show-sticky-counter");
  }
};
...
const updateCounterDisplay = (data) => {
  ...
  const stickyElements = {
    "sticky-visitor-count": data.total_visitors,
    "sticky-page-views": data.total_page_views,
  };

  Object.keys(stickyElements).forEach((elementId, index) => {
    const element = document.getElementById(elementId);
    if (element) {
      // Add staggered animation effect
      element.style.opacity = "0";
      setTimeout(() => {
        element.textContent = stickyElements[elementId].toLocaleString();
        element.style.opacity = "1";
